...
def api_delete_record(record_id):
    ...
    try:
        ...
        return jsonify({'success': False, 'message': str(e)}), 500
        # ===== 统计上个月总花销 =====
@app.route('/api/last_month_expense', methods=['GET'])
def api_last_month_expense():
    """统计上个月的支出总额"""
    user_email = get_user_from_request()
    if not user_email:
        return jsonify({'error': '未登录'}), 401
    ...
    try:
        ...
        count = 0
        for r in records:
            if r['date'].startswith(month_str) and r['type'] == '支出':
                total += r['amount']
                count += 1
...
# ===== 统计指定月份总花销 =====
@app.route('/api/month_expense', methods=['POST'])
def api_month_expense():
    """统计指定月份的支出总额"""
    user_email = get_user_from_request()
    if not user_email:
        return jsonify({'error': '未登录'}), 401
